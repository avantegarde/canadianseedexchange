...
function adace_after_snax_slot_option( $slot_fields, $adace_ad_slot ) {
	...
	$slot_fields['repeat_after_x_snax'] = esc_html__( 'Repeat after each X items', 'adace' );
...
function adace_afeter_snax_slot_inject( $snax_item, $snax_index ) {
	...
	$render_repeated_slot = $repeat_after > 0 && ( $inject_after === $current_post_number % $repeat_after );
