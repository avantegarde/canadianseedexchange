...
function snax_get_quiz_post_type() {
	return apply_filters( 'snax_quiz_post_type', 'snax_quiz' );
}
...
function snax_quiz_share_links( $quiz = null, $args = array() ) {
	...
	$defaults = array(
		'title' 		=> '',
		'url'			=> snax_get_share_url( $quiz ),
		'thumb'			=> get_the_post_thumbnail_url( $quiz ),
		'description'	=> '',
		'links'			=> apply_filters( 'snax_quiz_share_links', snax_get_share_position_active_networks( 'quiz_result' ) ),
	);
 ...
 * @return array
 */
function snax_allow_collections_for_quizzes( $post_types ) {
	$post_types[] = snax_get_quiz_post_type();

	return $post_types;
}

function snax_can_share_quiz() {
	$can = snax_shares_enabled() && snax_is_active_share_position( 'quiz_result' );

	$share_networks = snax_get_share_position_active_networks( 'quiz_result' );
...
function snax_can_share_to_unlock( $quiz = null ) {
	if ( ! snax_can_share_quiz() ) {
		return false;
	}
