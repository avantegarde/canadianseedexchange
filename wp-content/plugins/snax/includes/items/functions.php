...
function snax_delete_item( $item_id, $user_id = 0 ) {
	...
	$directly = true;
	do_action( 'snax_item_deleted', $post, $directly );
...
function snax_get_item_permalink( $item = null ) {
	...
	return apply_filters( 'snax_item_permalink', $permalink, $item );
}
...
function snax_get_items_query( $parent_format, $origin = 'all', $args = array() ) {
	...
	if ( isset( $args['post_parent'] ) ) {
		$default_args['post_parent'] = $args['post_parent'];
	}
...
function snax_get_items_query_args( $args = array() ) {
	...
	// If parent post is published, should show only published items.
	if ( ! isset( $args['force_status'] ) && isset( $args['post_parent'] ) && $args['post_parent'] > 0 ) {
...
function snax_show_item_voting_box( $post = null ) {
	...
	if ( snax_is_item( $post ) ) {
		...
		// Gallery items can't be voted.
		if ( 'gallery' === snax_get_format( $parent_id ) ) {
			$bool = false;
		}
...
function snax_show_item_share() {
	$show = snax_shares_enabled() && snax_is_active_share_position( 'list_item' );

	$networks = snax_get_share_position_active_networks( 'list_item' );
	...
	return (bool) apply_filters( 'snax_show_item_share', $show );
}
...
function snax_item_status_changed( $new_status, $old_status, $post ) {
	...
	do_action( 'snax_item_status_changed', $post );
}
...
function snax_return_item_comment_via_ajax( $location, $comment ) {
	...
	if ( $ajax_item_form ) {
		...
		$comments_args = apply_filters( 'snax_item_comments_args', $comments_args );

		$response = wp_list_comments( $comments_args, $comments );
		echo $response;
		echo '<script>(function(w) {if (w.addComment) {w.addComment.init(); } })(window);</script>';
		exit;
	}
	return $location;
}
...
/**
 * Update items counter on trashing item
 *
 * @param int $item_id    ID of the trashed Snax_item.
 */
function snax_trashed_item( $item_id ) {
    if ( snax_is_item( $item_id ) ) {
        $item_parent_id = wp_get_post_parent_id( $item_id );

        snax_update_post_submission_count( $item_parent_id );
    }
}

/**
 * Update items counter on untrashing item
 *
 * @param int $item_id    ID of the trashed Snax_item.
 */
function snax_untrashed_item( $item_id ) {
    if ( snax_is_item( $item_id ) ) {
        $item_parent_id = wp_get_post_parent_id( $item_id );
...
/**
 * Store item ID to update parent items counter later
 *
 * @param int $postid    ID of the Snax_item.
 */
function snax_before_delete_item( $postid ) {
    if ( snax_is_item( $postid ) ) {
        $item_parent_id = wp_get_post_parent_id( $postid );

        if ( $item_parent_id ) {
            global $snax_item_parent_ids_to_update;

            if ( ! isset( $snax_item_parent_ids_to_update ) ) {
                $snax_item_parent_ids_to_update = array();
            }

            $snax_item_parent_ids_to_update[ $postid ] = $item_parent_id;
        }
    }
}

/**
 * Update items counter on item deletion
 *
 * @param int $postid    ID of the Snax_item.
 */
function snax_deleted_item( $postid ) {
    // Item has been removed from database so we can't check its post type here.
    // We have to rely on the global var stored on the "before_delete_post" hook.
    global $snax_item_parent_ids_to_update;

    if ( empty( $snax_item_parent_ids_to_update ) ) {
        return;
    }

    foreach ( $snax_item_parent_ids_to_update as $item_id => $parent_id ) {
        snax_update_post_submission_count( $parent_id );

        unset( $snax_item_parent_ids_to_update[ $item_id ] );
    }
}
