...
function snax_votes_hidden() {
	$hidden = false;

	$count = snax_get_voting_score();

	if ( $count < 0 ) {
		$hidden = true;
	} else {
		$threshold = absint( snax_get_hide_votes_threshold() );
...
function snax_remove_vote( $item_id, $user_id ) {
	...
	do_action( 'snax_vote_removed', $vote_arr );
 ...
 * @param int $post_id      Post id.
 */
function snax_delete_post_votes( $post_id ) {
	global $wpdb;
	$table_name = $wpdb->prefix . snax_get_votes_table_name();
