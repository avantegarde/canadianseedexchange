...
function snax_handle_new_submission( $request ) {
	...
	if ( ! empty( $frontend_submission_nonce ) ) {
		...
		$tag_tax        = filter_input( INPUT_POST, 'snax-tags-taxonomy', FILTER_SANITIZE_STRING );
		$cat_tax        = filter_input( INPUT_POST, 'snax-categories-taxonomy', FILTER_SANITIZE_STRING );
		...
		$tags           = filter_input( INPUT_POST, 'snax-post-tags', FILTER_SANITIZE_STRING );
		...
		if ( empty( $status ) ) {
			$status = user_can( $author_id, 'snax_publish_posts', $post_id ) ? 'publish' : 'pending';
		}
		...
		if ( snax_is_category_field_required( $format ) && ! $category_selected ) {
			...
			if ( ! in_array( '', $allowed_categories, true ) ) {
				foreach ( $categories as $category_id ) {
					$category_obj = get_term( $category_id, $cat_tax );
		...
		$auto_assign = snax_get_category_auto_assign( $format );

		if ( is_array( $auto_assign ) && ! empty( $auto_assign ) ) {
			$auto_assign = array_filter( $auto_assign );

			foreach( $auto_assign as $category_slug ) {
				$auto_cat = get_term_by( 'slug', $category_slug, $cat_tax );
...
/**
 * Update the number of items belong to the post
 *
 * @param int|WP_Post $post         Optional. Post ID or WP_Post object. Default is global `$post`.
 *
 * @return int|false                Number of post items or false on failure.
 */
function snax_update_post_submission_count( $post = null ) {
	$post = get_post( $post );
    ...
    $submission_count = $query->found_posts;

	// Update.
	update_post_meta( $post->ID, '_snax_submission_count', $submission_count );

	do_action( 'snax_post_submission_count_updated', $post->ID, $submission_count );

	return $submission_count;
}

/**
 * Use first assigned Snax item with thumbnail as post featured image
 *
 * @param int $post_id      Post id.
 */
function snax_set_first_item_with_image_as_post_featured( $post_id = 0 ) {
	$parent = get_post( $post_id );

	// Get first image item.
	$items = snax_get_items( $parent->ID, array(
	...
	foreach ( $items as $item ) {
		...
		update_post_meta( $image_id, '_snax_featured_media_auto_set_from_item', $item->ID );
...
add_action( 'transition_post_status',  'snax_post_was_published', 10, 3 );
/**
 * Handle post publish.
 *
 ...
 */
function snax_post_was_published( $new_status, $old_status, $post ) {
	...
	// Items.
	if ( snax_is_item( $post->ID ) ) {
		do_action( 'snax_item_published', $post->ID );
		return;
	}
}
