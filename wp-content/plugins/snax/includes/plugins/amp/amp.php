...
function snax_amp_initialize() {
	if ( function_exists( 'is_amp_endpoint' ) && is_amp_endpoint() ) {
		...
		// This filter removes images from AMP post content.
		...
		add_filter( 'snax_enable_entry_action_links', '__return_false' );

		// Entire AMP page is the main loop.
		add_filter( 'snax_in_custom_loop', '__return_false', 99 );
	}
}
		...
		<div class="<?php echo implode( ' ', array_map( 'sanitize_html_class', $final_class ) ); ?>" data-snax-item-id="<?php echo absint( $post->ID ); ?>">
			...
			if ( snax_show_item_upvote_link( $post ) ) :
				...
				<form method="post"
					...
					target="_blank"
					id="snax-form-upvote-<?php echo esc_attr( $post->ID );?>"
					snax-amp-class="class<?php echo esc_attr( $post->ID );?>"
					class="<?php echo esc_attr( $current_vote );?>">
			...
			if ( snax_show_item_downvote_link( $post ) ) :
				...
				<form method="post"
					...
					on="submit-success:AMP.setState({count<?php echo esc_attr( $post->ID );?>: event.response.args.html}),AMP.setState({class<?php echo esc_attr( $post->ID );?>: event.response.args.class})"
					target="_blank"
					id="snax-form-downvote-<?php echo esc_attr( $post->ID );?>"
					snax-amp-class="class<?php echo esc_attr( $post->ID );?>"
					class="<?php echo esc_attr( $current_vote );?>">
				<input type="hidden" name="action" 			value="snax_amp_vote_item">
				<input type="hidden" name="security" 		value="<?php echo esc_attr( $nonce );?>">
