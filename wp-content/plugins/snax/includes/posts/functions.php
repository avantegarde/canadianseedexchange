	...
	switch ( $format ) {
		...
		case 'list':
			...
			foreach ( $post_items as $post_item ) {
				...
				$directly = false;
				do_action( 'snax_item_deleted', $post_item, $directly );
			}
			break;
	}
}
 ...
 */
function snax_close_post_submission( $post_id, $submission_count ) {
 ...
 * Delete a post
 *
 * @param WP_Post | int $post_id        Post id or object.
 *
 * @return bool | WP_Post
 */
function snax_delete_post( $post_id = 0 ) {
	$post = get_post( $post_id );
	...
	if ( false !== $deleted_post ) {
		do_action( 'snax_post_deleted', $deleted_post, $format );
	}
...
/**
 * Check whether post was deleted
 *
 * @return bool
 */
function snax_post_deleted() {
	if ( ! is_user_logged_in() ) {
		return false;
	}

	$url_var = snax_get_url_var( 'delete_status' );

	$deleted = filter_input( INPUT_GET, $url_var, FILTER_SANITIZE_STRING );
...
function snax_get_posts_query_args( $args = array() ) {
	...
	if ( apply_filters( 'snax_only_frontend_submitted_user_posts', false ) ) {
...
function snax_post_items_update_status( $post ) {
	$items = snax_get_items( $post, array(
...
/**
 * Update post submissions count when its item was approved
 *
 * @param int|WP_Post $item             Snax item.
 */
function snax_update_parent_submissions_count( $item ) {
...
function snax_get_field_values( $field_name = '' ) {
	$field_values = array(
		...
		'list_submissions'	=> '',
		'legal'         	=> false,
		'meme-top-text' 	=> __( 'Top text&hellip;', 'snax' ),
		'meme-bottom-text' 	=> __( 'Bottom text&hellip;', 'snax' ),
	);
 ...
 * @param string $taxonomy      Optional. Taxonomy of categories
 *
 * @return string
 */
function snax_get_post_included_categories( $format, $taxonomy = 'category' ) {
	$categories = '';
 ...
 * @return string
 */
function snax_get_post_excluded_categories( $taxonomy = 'category' ) {
	$categories = '';
 ...
 * @return array
 */
function snax_add_post_report_link( $args ) {
	if ( snax_report_post_abuse() ) {
		$args['links']['report_post'] = snax_post_report_link();
	}
...
/**
 * Allow comments on publish
 *
 * @param int $post_id      Published post id
 */
function snax_post_allow_comments( $post_id ) {
