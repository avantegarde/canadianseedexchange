...
/**
 * Class Snax_Widget_CTA_Button
 */
class Snax_Widget_CTA_Button extends WP_Widget {
	...
	function __construct() {
		$widget_options = apply_filters( 'snax_widget_cta_button_options', array(
			'classname'   => 'snax snax-widget-cta-button',
			'description' => esc_html__( 'Button with link to the Frontend Submission page', 'snax' ),
		) );

		parent::__construct( 'snax_widget_cta_button', esc_html__( 'Snax Call To Action Button', 'snax' ), $widget_options );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $instance['id'] ) ) {
			$instance['id'] = 'snax-widget-cta-button' . self::$counter++;
		}
		...
		do_action( 'snax_before_widget_cta_button_title' );
		...
		set_query_var( 'snax_cta_icon', $snax_cta_icon );
		set_query_var( 'snax_cta_button_label', $button_label );
		snax_get_template_part( 'widget-call-to-action-button' );

		do_action( 'snax_widget_cta_button_before_after_widget' );
	...
	public function form( $instance ) {
		...
		<div class="snax-widget-cta">
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'class' ) ); ?>"><?php esc_html_e( 'HTML class attribute (optional)', 'snax' ); ?>
					:</label>
				<input class="widefat"
				       type="text"
	...
	public function update( $new_instance, $old_instance ) {
		...
		do_action( 'wpml_register_single_string', 'Widgets', 'Snax Call to Action Button - button label field', $instance['button_label'] );
