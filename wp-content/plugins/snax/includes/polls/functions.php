...
function snax_get_poll_defaults( $name = '' ) {
    ...
    $allow_guests_to_play = get_option( 'snax_poll_allow_guests_to_play', 'standard' );

    // Convert checkbox value.
    if ( 'standard' !== $allow_guests_to_play ) {
        $allow_guests_to_play = 'none';
    }

    $vote_limit = get_option( 'snax_limits_poll_vote_limit', -1 );


	$defaults = array(
		'answers_set'	                => 'yes-no',
		'reveal_correct_wrong_answers'	=> 'standard',
		'one_question_per_page' 		=> 'none',
		'shuffle_questions' 			=> 'none',
		'questions_per_poll' 			=> '',
		'shuffle_answers' 				=> 'none',
		'allow_guests_to_play'			=> $allow_guests_to_play,
...
function snax_get_poll_setting( $name, $poll = null ) {
	...
	// Backward compatibility.
    if ( 'reveal_correct_wrong_answers' === $name ) {
        // Map old options (immediate, poll-end) into standard.
        if ( ! in_array( $setting, array( 'none', 'standard' ) ) ) {
            $setting = 'standard';
        }
    }
...
function snax_get_poll_post_type() {
	return apply_filters( 'snax_poll_post_type', 'snax_poll' );
}
...
function snax_poll_share_links( $poll = null, $args = array() ) {
	...
	$defaults = array(
		'title' 		=> '',
		'url'			=> snax_get_share_url( $poll ),
		'thumb'			=> get_the_post_thumbnail_url( $poll ),
		'description'	=> '',
		'links'			=> apply_filters( 'snax_poll_share_links', snax_get_share_position_active_networks( 'poll_result' ) ),
	);
...
/**
 * Check whether guests can play
 *
 * @param WP_Post $poll		Optional. Post object or id.
 *
 * @return string
 */
function snax_poll_allow_guests_to_play( $poll = null ) {
    $poll = get_post( $poll );

    $ret = snax_get_poll_setting( 'allow_guests_to_play', $poll );

    return 'standard' === $ret;
}

/**
 * Return user's vote limit
 *
 * @param WP_Post $poll		Optional. Post object or id.
 *
 * @return string
 */
function snax_poll_vote_limit( $poll = null ) {
    $poll = get_post( $poll );

    $ret = snax_get_poll_setting( 'vote_limit', $poll );
...
function snax_poll_share_to_unlock( $poll = null ) {
	if ( ! snax_can_share_poll_to_unlock() ) {
		return false;
	}
...
function snax_remove_poll_children( $post_id ) {
	...
	$args['post_type'] = snax_get_poll_result_post_type();

	$children = array_merge( $children, get_children( $args ) );

	$args['post_type'] = snax_get_poll_answer_post_type();
 ...
 *
 * @param int|WP_Post $poll_id	Post object or id.
 * @param int	      $question_id	The question to share.
 *
 * @return string
 */
function snax_get_poll_share_links( $poll_id, $question_id ) {

	if ( ! snax_can_share_poll_question() ) {
		return '';
	}
	...
	$url 		 = snax_get_share_url( $poll_id );
	...
	if ( ! $question['title_hide'] ) {
		$snax_share_args['description'] = $question['title'] . '. ' . esc_html__( 'My choice: ', 'snax' ) . 'MY_CHOICE';
	} else{
        $snax_share_args['description'] = esc_html__( 'My choice: ', 'snax' ) . 'MY_CHOICE';
    }
...
/**
 * Enabled collections support
 *
 * @param array $post_types     Allowed post types.
 *
 * @return array
 */
function snax_allow_collections_for_polls( $post_types ) {
	$post_types[] = snax_get_poll_post_type();

	return $post_types;
}

function snax_can_share_poll() {
	$can = snax_shares_enabled() && snax_is_active_share_position( 'poll_result' );

	$share_networks = snax_get_share_position_active_networks( 'poll_result' );

	// No networks to share?
...
function snax_can_share_poll_to_unlock() {
	if ( ! snax_can_share_poll() ) {
		return false;
	}

	$share_networks = snax_get_share_position_active_networks( 'poll_result' );

	// Share to Unlock is based on Facebook. Without it, it's not possible to use unlock option.
	return in_array( 'facebook', $share_networks );
}

function snax_can_share_poll_question() {
	$can = snax_shares_enabled() && snax_is_active_share_position( 'poll_question' );

	$share_networks = snax_get_share_position_active_networks( 'poll_question' );
