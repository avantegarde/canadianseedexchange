<?php
/**
 * New post form for format "Binary Poll"
 *
 * @package snax 1.11
 * @subpackage Theme
 */
...
<?php do_action( 'snax_before_frontend_submission_form', $snax_post_format ); ?>
	...
	<form action="" method="post" class="<?php echo implode( ' ', array_map( 'sanitize_html_class', $snax_class ) ); ?>">
		...
		<div class="snax-form-main">
			...
			wp_enqueue_script( 'snax-binary-poll',  snax_get_includes_url() . 'polls/admin/js/binary-poll.js', array( 'backbone', 'snax-poll-common', 'jquery', 'jquery-ui-sortable' ), snax_get_version(), true );
			...
			wp_localize_script( 'snax-binary-poll', 'snax_binary_poll_config', wp_json_encode( $poll_config ) );
			...
			if ( $snax_post_id ) {
				...
				$poll_post = snax_get_user_draft_poll( 'binary' );
			}
			...
			snax_get_template_part( 'polls/binary/form-frontend/poll-tpl' );
		...
		<div class="snax-form-side">
			...
			<input type="hidden" name="snax-post-format" value="binary_poll"/>
